...
function populateFarmCalcData() {
	...
	if (universe === 1 && game.upgrades.Formations.done) {
		...
		if (runningAutoTrimps ? getPageSetting('autoLevelScryer') : true) {
			if (['Wind', 'ALL'].includes(game.global.uberNature) && empowerment !== 'Wind') stances += 'W';
	...
	return {
		...
		wind: 0,
		ice: 0,
		windCap: ['Wind', 'ALL'].includes(uberEmpowerment) ? 300 : 200,
		natureIncrease: 1 * (uberEmpowerment === empowerment && game.global.uberNature !== 'Poison' ? 2 : 1) * plaguebrought /* Maybe buggy atm. Might need an ALL check here? */,
	...
	while (ticks < max_ticks) {
		...
		while (enemyHealth >= 1 && ticks < max_ticks) {
			...
			if (!armyDead()) {
				...
				if (saveData.poison) poison += trimpAttack * (['Poison', 'ALL'].includes(saveData.uberNature) ? 2 : 1) * saveData.natureIncrease;
