...
function dropPrestiges() {
	if (challengeActive('Finale')) return;
...
function breed() {
	...
	if (breeding.cmp(2) === -1 || challengeActive('Trapper') || challengeActive('Trappapalooza') || challengeActive('Finale')) {
		updatePs(0, true);
	...
	let potencyModifiers = {
		...
		mutGeneAttack: u2Mutations.tree.GeneAttack.purchased,
		mutGeneHealth: u2Mutations.tree.GeneHealth.purchased
	};
...
function setVoidCorruptionIcon(regularMap) {
	if (usingRealTimeOffline || game.talents.tier11b.purchased) return;
...
function handlePoisonDebuff() {
	...
	if (getEmpowerment() !== 'Poison' && (!Fluffy.isRewardActive('FluffyE14') || !challengeActive('Finale'))) {
	...
	if (isTrimp) {
		...
		if (game.global.universe === 1) {
			...
			const multipliers = {
				...
				poisionEmpowerment: () => (['Poison', 'ALL'].includes(getUberEmpowerment()) ? 3 : 1),
			...
			const challengeMultipliers = {
				...
				Finale: () => {
					let result = 1;
					if (Fluffy.isRewardActive('FluffyE10')) result = Math.pow(Fluffy.isRewardActive('FluffyE16') ? 5 : 3.1, Fluffy.getCurrentPrestige());
					if (Fluffy.isRewardActive('FluffyE13')) result *= 11;
					if (Fluffy.isRewardActive('FluffyE17')) result *= 1 + game.global.magmite;
					if (Fluffy.isRewardActive('FluffyE19')) result *= 1 + playerSpire.spirestones / 100;
					return result;
				}
			};
		...
		if (game.global.universe === 2) {
			const multipliers = {
				...
				spireNonDaily: () => (!challengeActive('Daily') && Fluffy.isRewardActive('Scruffy32') ? Fluffy.rewardConfig.SADailies.attackMod() : 1)
			};
			number = applyMultipliers(multipliers, number);
			...
			const challengeMultipliers = {
				...
				Doubleless: () => game.challenges.Smithless.getTrimpMult()
			};
			number = applyMultipliers(challengeMultipliers, number, true);
		}
		if (challengeActive('Daily')) {
...
function buildBuilding(what, amt = 1) {
	...
	if (!game.buildings.Hub.locked) {
		const amount = game.buildings.Hub.getAmount();
...
function startFight() {
	...
	if (cell.name === 'Improbability' && game.global.spireActive && (game.global.world >= 200 || !challengeActive('Spired'))) {
		...
		if (badCoord) displayedName = "<span class='smallEnemyName'>Druopitinity and Pals</span>";
	} else if (cell.name === 'Druopitinity' && !game.global.spireActive) {
		displayedName = 'Obsidimp';
		if (badCoord) displayedName = 'Obsidimps';
	...
	if ((challengeActive('Smithless') || challengeActive('Doubleless')) && !game.global.mapsActive && game.global.world % 25 === 0 && game.global.lastClearedCell === -1 && !cell.failedUber) {
	...
	if (cell.name === 'Omnipotrimp' && game.global.world % 5 === 0 && !game.global.spireActive && !(Fluffy.isRewardActive('FluffyE13') && challengeActive('Finale'))) {
	...
	if (cell.maxHealth === -1 && game.global.spireActive && !game.global.mapsActive && cell.corrupted) {
		if (Fluffy.isRewardActive('FluffyE15')) {
			if (cell.corrupted !== 'obsidian') cell.corrupted = 'none';
		} else if (checkIfSpireWorld()) {
	...
	if (cell.maxHealth === -1) {
		...
		if (game.global.universe === 1) {
			...
			} else if (challengeActive('Obliterated') || challengeActive('Eradicated') || (challengeActive('Liquified') && cell.name == 'Liquimp')) {
			...
			if (game.global.mapsActive && game.global.world >= corruptionStart && !game.talents.tier11b.purchased) {
		...
		if (cell.health < 1) {
			...
			if (game.global.universe === 1) {
				...
				if (['Ice', 'ALL'].includes(empowermentUber)) overkillerCount += 2;
			...
			if (cell.plaguebringer) {
				...
				if (empowerment) {
					if (empowerment === 'Poison') {
						stackPoison(cell.plaguebringer); /* stackPoison handles the poison debuff and plaguebrought scaling */
					}

					if (empowerment === 'Wind') {
						let hits = cell.plagueHits;
						if (['Wind', 'ALL'].includes(empowermentUber)) hits *= 2;
					...
					if (empowerment === 'Ice') {
						let hits = cell.plagueHits;
						if (['Ice', 'ALL'].includes(empowermentUber)) hits *= 2;
						if (Fluffy.isRewardActive('plaguebrought')) hits *= 2;
						game.empowerments[empowerment].currentDebuffPower += Math.ceil(hits);
						handleIceDebuff();
					}
	...
	if (game.global.soldierHealth <= 0) {
		...
		if (getPerkLevel('Anticipation')) {
			...
			const maxStacks = challengeActive('Finale') ? 0 : game.talents.patience.purchased ? 45 : 30;
		...
		if (magmaActive && !(Fluffy.isRewardActive('FluffyE13') && challengeActive('Finale'))) {
		...
		if (game.global.universe === 2) {
			...
			if (Fluffy.isRewardActive('Scruffy28')) game.global.soldierHealthMax *= Fluffy.getDamageModifier();
			...
			if ((challengeActive('Smithless') || challengeActive('Doubleless')) && game.challenges.Smithless.fakeSmithies > 0) {
		...
		if (game.global.universe === 1) {
			game.global.soldierHealthMax = Math.min(game.global.soldierHealthMax, 1e300);
		...
		if (game.global.difs.health !== 0) {
			...
			if (magmaActive && !(Fluffy.isRewardActive('FluffyE13') && challengeActive('Finale'))) healthTemp *= mutations.Magma.getTrimpDecay();
		...
		if (game.global.difs.attack !== 0) {
			...
			if (magmaActive && !(Fluffy.isRewardActive('FluffyE13') && challengeActive('Finale'))) attackTemp *= mutations.Magma.getTrimpDecay();
	...
	if (game.global.soldierHealth > game.global.soldierHealthMax) game.global.soldierHealth = game.global.soldierHealthMax; /* THIS NEEDS SORTED ASAP. I made a mess of it */
...
function fight(makeUp) {
	...
	if (cell.health <= 0 || !isFinite(cell.health)) {
		...
		if ((challengeActive('Smithless') || challengeActive('Doubleless')) && cell.ubersmith && !cell.failedUber) {
	...
	if ((challengeActive('Smithless') || challengeActive('Doubleless')) && cell.ubersmith && !cell.failedUber) checkFast = true;
	...
	if (trimpAttack > 0 && checkFast && !forceSlow) {
		...
		if (game.global.soldierHealth > 0) {
			if (!badDodge) {
				if (empowerment === 'Poison' || challengeActive('Finale')) {
					cell.health -= game.empowerments.Poison.getDamage();
					if (empowerment === 'Poison') stackPoison(trimpAttack);
				}
		...
		if (cell.health < 1 && game.global.formation === 5 && empowerment === 'Wind' && ['Wind', 'ALL'].includes(empowermentUber) && game.empowerments.Wind.currentDebuffPower < game.empowerments.Wind.stackMax()) {
			cell.health = 1;
		}
	...
	if (game.global.soldierHealth > 0 && getHeirloomBonus('Shield', 'gammaBurst') > 0 && (cell.name !== 'Druopitinity' || !game.global.spireActive)) {
		...
		if (burst.stacks >= triggerStacks) {
			...
			if (cell.health > 0) {
				...
				if (game.global.formation === 5 && empowerment === 'Wind' && ['Wind', 'ALL'].includes(empowermentUber) && cell.health < 1) {
					cell.health = 1;
				...
				if (Fluffy.isRewardActive('FluffyE15') && challengeActive('Finale')) {
					if (empowerment === 'Poison') {
	...
	if (game.global.formation === 5 && empowerment === 'Wind' && ['Wind', 'ALL'].includes(empowermentUber)) {
		overkill = 0;
		if (plaguebringer === 0) plaguebringer = 1;
	}

	if (cell.health / cell.maxHealth < 0.9 && challengeActive('Finale') && Fluffy.isRewardActive('FluffyE18')) {
		cell.health = 0;
		thisKillsTheBadGuy();
		overkill = 'shatter';
	} else if (cell.health / cell.maxHealth < 0.5 && empowerment === 'Ice' && ['Ice', 'ALL'].includes(empowermentUber) && game.empowerments.Ice.currentDebuffPower > 20) {
		cell.health = 0;
		thisKillsTheBadGuy();
		overkill = 'shatter';
	}
	...
	if (game.global.universe === 1) {
		...
		if (empowerment === 'Ice' && attacked) {
			let addStacks = 1;
			if (['Ice', 'ALL'].includes(empowermentUber)) addStacks *= 2;
		...
		if (empowerment === 'Wind' && attacked) {
			let addStacks = 1;
			if (['Wind', 'ALL'].includes(empowermentUber)) addStacks *= 2;
			if (Fluffy.isRewardActive('plaguebrought')) addStacks *= 2;
	...
	if (game.global.universe === 2) {
		...
		if ((challengeActive('Smithless') || challengeActive('Doubleless')) && cell.ubersmith) {
...
function nextWorld() {
	...
	if (game.talents.bionic.purchased && game.global.universe === 1) {
		const bTier = Math.min((game.global.world - 126) / 15, 52.9);
	...
	if (game.global.world === 236 && ['Wind', 'ALL'].includes(getUberEmpowerment())) unlockFormation(5);
	...
	if (game.global.world >= getObsidianStart()) {
		let next = game.global.highestRadonLevelCleared >= 349 ? '50' : game.global.highestRadonLevelCleared >= 99 ? '25' : '10';
		let text;
	...
	for (let item in game.badGuys) {
		...
		if (level === totalCells && badGuy.last && (locationMatch || (!mapSuffix && badGuy.location === 'World')) && world >= badGuy.world) {
			...
			if (!mapSuffix && improbCheck && item === 'Blimp') {
				if (world === 900) item = 'Druopitinity';
				else if (magmaActive || world > 900) item = 'Omnipotrimp';
