...
function graphsDebug(message) {
	if (enableGraphsDebug) console.debug(...arguments);
}
...
function loadGraphData() {
	...
	for (const graph of graphList) {
		if (graph.toggles) {
			if (GRAPHSETTINGS.toggles[graph.id] === undefined) {
				GRAPHSETTINGS.toggles[graph.id] = {};
			}
			graph.toggles.forEach((toggle) => {
				if (GRAPHSETTINGS.toggles[graph.id][toggle] === undefined) {
					GRAPHSETTINGS.toggles[graph.id][toggle] = false;
				}
			});
		}
	}
...
function clearData(keepN, clrall = false) {
	...
	if (clrall) {
		...
		for (const [portalID, portalData] of Object.entries(portalSaveData)) {
			if (portalData.totalPortals != currentPortalNumber && portalData.universe == universe) {
				// only delete currently selected universe data
		...
		// keep keepN portals in selected universe , delete the rest
		var portals = Object.entries(portalSaveData)
			.filter((data) => data[1].universe == universe)
			.map((data) => {
				return data[0];
			});
...
function deleteSpecific() {
	...
	} // keep X portals, delete the rest
	else {
		for (const [portalID, portalData] of Object.entries(portalSaveData)) {
			if (portalData.totalPortals === portalNum && portalData.universe == GRAPHSETTINGS.universeSelection) {
				// only delete if in selected universe
				delete portalSaveData[portalID];
...
function Graph(dataVar, universe, selectorText, additionalParams = {}) {
	...
	this.lineGraph = function () {
		...
		var maxS3 = Math.max(
			...Object.values(portalSaveData)
				.map((portal) => portal.s3)
				.filter((s3) => s3)
		);
		var activeToggles = [];
		if (this.toggles) {
		...
		activeToggles.forEach((toggle) => {
			if (toggledGraphs[toggle].dataVars) activeDataVars.push(toggledGraphs[toggle].dataVars);
		});
		var portalCount = 0;
		...
		for (const portal of Object.values(portalSaveData).reverse()) {
			...
			for (const index in portal.perZoneData[item]) {
				...
				for (var toggle of activeToggles) {
					...
					try {
						x = toggledGraphs[toggle].customFunction(portal, item, index, x, time, maxS3, xprev);
					} catch (e) {
						x = 0;
	...
	this.columnGraph = function () {
		...
		highChartsObj.plotOptions.series = { groupPadding: 0.2, pointPadding: 0, animation: false, borderColor: 'black' };
		...
		if (GRAPHSETTINGS.toggles[this.id].perHr) {
			// disable time when comparing things over time.  x/x is not interesting data.
		...
		for (const column of activeColumns) {
			...
			for (const portal of Object.values(currUniPortals).slice(Math.max(Object.values(currUniPortals).length - GRAPHSETTINGS.portalsDisplayed, 0))) {
				...
				var data = undefined;
				if (portal[column.dataVar]) {
					data = portal[column.dataVar];
				}
				...
				if (column.customFunction) {
					data = column.customFunction(portal, data);
				}
				if (GRAPHSETTINGS.toggles[this.id].perHr) {
					// HACKS a headache for future me if other toggles are wanted here.
			...
			if (hasData) {
				// only add columns if there is any data in that column over all portals
				var series = {
					name: column.title,
					data: cleanData,
					type: 'column',
				...
				if (column.dataVar === 'currentTime') {
					// HACKS override formatter for time vars
		...
		// Label the axes
		var axes = displayedColumns.map((column) => {
			return { visible: false, endOnTick: false };
		});
		// reduce padding between portals as portals increase
		highChartsObj.plotOptions.series['groupPadding'] = 0.5 / this.graphData[0].data.length ** 0.6;
	...
	for (const graph of graphList) {
		...
		for (const universe of universes) {
			...
			for (portal of Object.values(portalSaveData)) {
				if (
					portal.perZoneData[graph.dataVar] &&
					portal.universe === universe && // has collected data, in the right universe
					new Set(portal.perZoneData[graph.dataVar].filter((x) => x === 0 || x)).size > 1
				) {
...
function applyRememberedSelections() {
	...
	for (var i = 0; i < chart1.series.length; i++) {
		if (GRAPHSETTINGS.rememberSelected[i] === false) {
...
function Portal() {
	...
	this.challenge =
		getGameData.challengeActive() === 'Daily'
	...
	this.update = function (fromMap) {
		// check source of the update
		...
		for (const [name, data] of Object.entries(this.perZoneData)) {
			if (world + 1 < data.length) {
				// FENCEPOSTING (zones are 1 indexed)
...
function pushData(fromMap) {
	...
	if (!portalSaveData[portalID] || getGameData.world() === 1) {
		// reset portal data if restarting a portal
...
const getGameData = {
	...
	runetrinkets: () => {
		return game.stats.runetrinkets.value;
	},
	chalBalance: () => {
		return game.challenges.Balance.balanceStacks;
	},
	chalStacks: () => {
...
const graphList = [
	...
	new Graph('fluffy', 1, 'Fluffy Exp', {
		conditional: () => {
			return getGameData.u1hze() >= 299 && Fluffy.currentLevel !== 10;
		}, // pre unlock, post E10L10
		customFunction: (portal, i) => {
			return diff('fluffy', portal.initialFluffy)(portal, i);
		},
		toggles: ['perHr', 'perZone']
	}),
	new Graph('essence', 1, 'Dark Essence', {
		conditional: () => {
			return getGameData.essence() < 5.826e39;
		},
		customFunction: (portal, i) => {
			return diff('essence', portal.initialDE)(portal, i);
		},
		toggles: ['perHr', 'perZone'],
		xminFloor: 181
	}),
	new Graph('lastWarp', 1, 'Warpstations', {
		...
		conditional: () => {
			return getGameData.u1hze() >= 59 && game.global.totalHeliumEarned - game.global.heliumLeftover < 10 ** 10;
		}, // Warp unlock, less than 10B He allocated
		xminFloor: 60
	}),
	new Graph('amals', 1, 'Amalgamators'),
	new Graph('wonders', 1, 'Wonders', {
		conditional: () => {
			return getGameData.challengeActive() === 'Experience';
		},
		xminFloor: 300
	}),
	...
	new Graph('scruffy', 2, 'Scruffy Exp', {
		customFunction: (portal, i) => {
			return diff('scruffy', portal.initialScruffy)(portal, i);
		},
		toggles: ['perHr', 'perZone']
	}),
	new Graph('runetrinkets', 2, 'Runetrinkets', {
		conditional: () => {
			return getGameData.u2hze() >= 130;
		}, // unlock (close enough)
		toggles: ['perHr', 'perZone'],
		xminFloor: 100
	}),
	new Graph('mutatedSeeds', 2, 'Mutated Seeds', {
		conditional: () => {
			return getGameData.u2hze() >= 200;
		},
		customFunction: (portal, i) => {
			return diff('mutatedSeeds', portal.initialMutes)(portal, i);
		},
		toggles: ['perHr', 'perZone'],
		xminFloor: 200
	}),
	...
	new Graph('bonfires', 2, 'Bonfires', {
		...
		conditional: () => {
			return getGameData.challengeActive() === 'Hypothermia';
		}
	}),
	...
	new Graph('cruffys', 2, 'Cruffys', {
		conditional: () => {
			return getGameData.challengeActive() === 'Nurture';
		}
	}),
	...
	new Graph('c23increase', false, 'C2 Bonus', {
		conditional: () => {
			return game.global.runningChallengeSquared;
		},
	...
	new Graph('overkill', false, 'Overkill Cells', {
		...
		conditional: () => {
			return (getGameData.universe() == 1 && getGameData.u1hze() >= 169) || (getGameData.universe() == 2 && getGameData.u2hze() >= 200);
		}
	}),
	new Graph('mapbonus', false, 'Map Bonus'),
	new Graph('empower', false, 'Empower', {
		conditional: () => {
			return getGameData.challengeActive() === 'Daily' && typeof game.global.dailyChallenge.empower !== 'undefined';
		}
	}),
	new Graph('chalStacks', false, 'Challenge Stacks', {
		conditional: () => {
			return Object.keys(stackChallenges).some((chal) => {
				return challengeActive(chal);
			});
		}
	}),
	new Graph(false, false, 'Portal Stats', {
		...
		columns: [
			...
			{ dataVar: 'radonOwned', universe: 2, title: 'Radon', color: '#5bc0de' },
			{
				...
				title: 'Pet Exp',
				color: 'green',
				customFunction: (portal, x) => {
					return x - portal.initialFluffy;
				}
			},
			{
				dataVar: 'scruffy',
				...
				color: 'green',
				customFunction: (portal, x) => {
					return x - portal.initialScruffy;
				}
			},
			...
			},
			{
				...
				color: '#a16e08',
				customFunction: (portal, x) => {
					return portal.perZoneData.mapbonus.length - 1;
				}
			},
	...
	*/
	...
	lifetime: {
		...
		customFunction: (portal, item, index, x) => {
			var initial;
			if (item === 'heliumOwned') {
				initial = portal.totalHelium;
			}
			if (item === 'radonOwned') {
				initial = portal.totalRadon;
			}
			if (item === 'c23increase') {
				initial = portal.cinf;
			}
...
nextWorld = function () {
	try {
		...
		graphsDebug('Gather info failed: ', e);
	}
...
rewardResource = function () {
	if (arguments[0] === 'helium' && game.global.mapsActive) {
		try {
			...
			graphsDebug('Gather info failed: Cthulimp: ', e);
		}
	}
	...
	if (arguments[0] === 'helium' && game.global.mapsActive) {
		try {
			...
			partialPushData([
				['mapHeRn', final - initial, true, true],
				[gas, final, false, false]
			]); // update both map He and total He
		} catch (e) {
			graphsDebug('Gather info failed: Cthulimp: ', e);
		}
	}
...
buyUpgrade = function () {
	...
	if (arguments[0] == 'Gigastation' && output) {
		try {
			...
			graphsDebug('Gather info failed: Warps per Giga: ', e);
		}
	}
};
