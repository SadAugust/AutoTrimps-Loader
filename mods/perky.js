...
MODULES.autoPerks = {
	...
	displayGUI: function (universe = portalUniverse, forceRefresh = false) {
		...
		if (MODULES.autoPerks.loaded === calcName && targetZoneElem) {
		...
		if (!forceRefresh && MODULES.autoPerks.GUI && MODULES.autoPerks.GUI && MODULES.autoPerks.GUI.$preset && !compareSelectOptions(selectElement, MODULES.autoPerks.GUI.$preset)) {
			forceRefresh = true;
		}
		...
		for (let x = 0; x < Object.keys(inputBoxes).length; x++) {
			...
			for (let item in inputBoxes[row]) {
				MODULES.autoPerks.createInput(inputContainer, item, inputBoxes[row][item], settingInputs && settingInputs[item] !== null ? settingInputs[item] : 1, calcName);
...
if (typeof autoTrimpSettings === 'undefined' || (typeof autoTrimpSettings !== 'undefined' && typeof autoTrimpSettings.ATversion !== 'undefined' && !autoTrimpSettings.ATversion.includes('SadAugust'))) {
	...
	(async function () {
		let basepathPerkCalc = 'https://sadaugust.github.io/AutoTrimps-Loader/';
		if (typeof localVersion !== 'undefined') basepathPerkCalc = 'https://localhost:8887/AutoTrimps-Loader/';
		const mods = ['surky'];
