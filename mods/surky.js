...
function getPerkEfficiencies(props, perks) {
	...
	// Resourceful
	const resourceful = 1 + perks.Resourceful.level * perks.Resourceful.effect;
	let resourcefulGain = (resourceful + perks.Resourceful.effect) / resourceful;
	// trappa is heavily drop-based prior to 160, and mostly gathering based after 170
	if (props.specialChallenge === 'trappa') {
		if (props.targetZone < 162) {
			resourcefulGain = Math.pow(resourcefulGain, 0.0001);
			// derate resourceful in trappa (don't want to zero it out but it doesn't increase F/M/L meaningfully)
		} else if (props.targetZone < 172) {
			resourcefulGain = Math.pow(resourcefulGain, 0.5);
		}
	}

	const resourcefulCost = getPerkCost('Resourceful', 1, false, perks);
	// Resourceful:
	//   all-building cost reduction
	perks.Resourceful.efficiency = getLogWeightedValue(props, perks, 1, 1, 1, resourcefulGain, 1, 1) / resourcefulCost;
