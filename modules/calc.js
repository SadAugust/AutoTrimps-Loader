...
function calcEquipment(equipType = 'attack', extraItem = new ExtraItem()) {
	...
	if (equipType === 'health' && challengeActive('Finale')) return 5e201;
	...
	for (let name of equipmentList) {
		...
		if (name === extraItem.name) {
			...
			const newStatsPerLevel = prestigeLevel ? Math.round(equip[equipType] * Math.pow(1.19, Math.max(0, prestigeLevel - 1 - game.global.druopitinityDefeated) * game.global.prestige[equipType] + 1)) : equip[equipType];
...
function calcSpire(what = 'attack', cell, name, checkCell = false) {
	...
	if (enemy === 'Druopitinity') return what === 'attack' ? 0 : 1e300;
	...
	if (challengeActive('Spired')) {
		if (game.global.world === 200 && enemy === 'Improbability') return 1e165; /* Druopitee's health in Spired challenge is fixed to 1e165 */
		base *= 1e20;
...
function getTrimpHealth(realHealth, worldType = _getWorldType(), extraItem = new ExtraItem()) {
	...
	if (game.global.universe === 1) {
		const healthMultipliers = {
			...
			magma: () => (!(Fluffy.isRewardActive('FluffyE13') && challengeActive('Finale')) ? mutations.Magma.getTrimpDecay() : 1),
	...
	if (game.global.universe === 2) {
		const healthMultipliers = {
			...
			Scruffy28: () => (Fluffy.isRewardActive('Scruffy28') ? Fluffy.getDamageModifier() : 1)
		};
		health = applyMultipliers(healthMultipliers, health);
		...
		const challengeMultipliers = {
			...
			Doubleless: () => (game.challenges.Smithless.fakeSmithies > 0 ? Math.pow(1.25, game.challenges.Smithless.fakeSmithies) : 1)
		};
...
function calcOurBlock(stance = false, realBlock = false, worldType = _getWorldType(), extraGyms = 0, extraItem = new ExtraItem()) {
	...
	if (game.global.infblock) return Infinity;
	...
	if (shield.blockNow) {
		if (extraItem.name === 'Shield') {
			...
			const newStatsPerLevel = Math.round(shield.block * Math.pow(1.19, Math.max(0, prestigeLevel - 1 - game.global.druopitinityDefeated) * game.global.prestige['block'] + 1));
...
function getTrimpAttack(realDamage) {
	...
	if (mutations.Magma.active() && !(Fluffy.isRewardActive('FluffyE13') && challengeActive('Finale'))) attack *= mutations.Magma.getTrimpDecay();
...
function calcOurDmg(minMaxAvg = 'avg', universeSetting, realDamage = false, worldType = _getWorldType(), critMode, mapLevel = _getZone(worldType) - game.global.world, useTitimp = false, specificHeirloom = false) {
	...
	if (game.global.universe === 2) {
		const damageModifiers = {
			...
			spireMult: () => (Fluffy.isRewardActive('Scruffy32') && !challengeActive('Daily') ? Fluffy.rewardConfig.SADailies.attackMod() : 1)
		};
		attack = applyMultipliers(damageModifiers, attack);
...
function calcEnemyBaseAttack(worldType = _getWorldType(), zone = _getZone(worldType), cell = _getCell(), name = _getEnemyName('Chimp'), query = false) {
	...
	if (game.global.druopitinityDefeated) attack /= Math.pow(1.19, 14 * game.global.druopitinityDefeated);
	if (zone === 1) {
		attack *= 0.35;
	...
	if (game.global.universe === 2) {
		...
		attack *= Math.pow(1.5, part1) * Math.pow(1.4, part2) * Math.pow(1.32, part3);
	}

	if (name) attack *= game.badGuys[name].attack;

	return attack < 1 ? 1 : Math.floor(attack);
}
...
function calcEnemyAttackCore(worldType = _getWorldType(), zone = _getZone(worldType), cell = _getCell(), name = _getEnemyName('Snimp'), minOrMax = false, customAttack, equality) {
	...
	if (customAttack) {
		...
		} else if (!game.talents.tier11b.purchased) {
	...
	if (game.global.universe === 1) {
		const challengeMultipliers = {
			...
			Liquified: () => _getObliteratedStatMultiplier('Liquified')
		};
		attack = applyMultipliers(challengeMultipliers, attack, true);
	}
...
function calcSpecificEnemyAttack(critPower = 2, customBlock, customHealth, customAttack) {
	...
	if (game.global.universe === 1 && !game.global.mapsActive && (corrupt || (enemy.level === 100 && mutations.Corruption.active()) || challengeActive('Spired'))) {
		if (enemy.level === 100) {
			if (!game.global.spireActive && !Fluffy.isRewardActive('FluffyE15')) attack *= calcCorruptionScale(game.global.world, 3);
...
function calcEnemyBaseHealth(worldType = _getWorldType(), zone = _getZone(worldType), cell = _getCell(), name = _getEnemyName('Turtlimp'), ignoreMutation) {
	...
	if (game.global.druopitinityDefeated) health /= Math.pow(1.19, 13 * game.global.druopitinityDefeated);
...
function calcSpecificEnemyHealth(worldType = _getWorldType(), zone = _getZone(worldType), cell = _getCell(), forcedName) {
	...
	} else if (game.global.universe === 1 && (corrupt || (cell === 100 && mutations.Corruption.active()) || challengeActive('Spired'))) {
		if (cell === 100 && !game.global.spireActive && !Fluffy.isRewardActive('FluffyE15')) health *= calcCorruptionScale(game.global.world, 10);
...
function calcHitsSurvived(targetZone = _getZone(), worldType = _getWorldType(), difficulty = 1, extraGyms = 0, extraItem = new ExtraItem(), checkOutputs) {
	...
	let block = calcOurBlock(false, false, worldType, extraGyms, extraItem) / formationMod;
	if (block === -Infinity) block = 0;
...
function corruptionBaseStats(cell = game.global.lastClearedCell + 1, targetZone = game.global.world, type = 'attack', ignoreMultiplier = false, e15 = Fluffy.isRewardActive('FluffyE15')) {
	cell = game.global.gridArray[cell];
	const typeFunction = type === 'attack' ? calcEnemyBaseAttack : calcEnemyBaseHealth;
	let baseStats = typeFunction('world', targetZone, cell.level, e15 ? cell.name : 'Chimp', true);
	if (type === 'attack') {
		if (!ignoreMultiplier) {
			if (cell.corrupted === 'corruptStrong') baseStats *= 2;
			else if (cell.corrupted === 'healthyStrong') baseStats *= 2.5;
		}
		if (!e15) baseStats *= cell.mutation === 'Healthy' ? calcCorruptionScale(targetZone, 5) : calcCorruptionScale(targetZone, 3);
	} else if (type === 'health') {
		if (!ignoreMultiplier) {
			if (cell.corrupted === 'corruptTough') baseStats *= 5;
			else if (cell.corrupted === 'healthyTough') baseStats *= 7.5;
		}
		if (!e15) baseStats *= cell.mutation === 'Healthy' ? calcCorruptionScale(targetZone, 14) : calcCorruptionScale(targetZone, 10);
	}
...
function calcCorruptedStats(targetZone = game.global.world, type = 'attack') {
	...
	const e15 = Fluffy.isRewardActive('FluffyE15');
	const gridArray = game.global.gridArray;
	let stat;
	gridArray.forEach((grid, i) => {
		if (grid.mutation) {
			const enemyStat = corruptionBaseStats(i, targetZone, type, e15);
...
function mutationBaseStats(cell = game.global.lastClearedCell + 1, targetZone = game.global.world, type = 'attack', randomized = challengeActive('Randomized')) {
	...
	if (randomized) baseStats *= Math.pow(statMult, 200) * 400;
	return baseStats;
}
...
function calcMutationStats(targetZone = game.global.world, type = 'attack') {
	...
	const runningRandomized = challengeActive('Randomized');
	...
	for (let i = 0; i < gridArray.length; i++) {
		...
		if (mutation && mutation.length) {
			let enemyStats = mutationBaseStats(i, targetZone, type, runningRandomized);
...
function enoughHealth(map, minAvgMax = 'max') {
	...
	let block = calcOurBlock(false, false);
	if (block === -Infinity) block = 0;
