...
function maxOneShotPower(planToMap = false, targetZone = game.global.world) {
	...
	if (game.global.universe === 1) {
		...
		if (empowerment === 'Ice') {
			...
			if (['Ice', 'ALL'].includes(uberEmpowerment)) power += 2;
		...
		if (!overkill) {
			if (u2Mutations.tree.MadMap2.purchased) {
...
function unlockedStances() {
	...
	if (game.global.uberNature === 'Wind' || game.global.fluffyPrestige >= 15 || Fluffy.isRewardActive('FluffyE14')) stances.push('W');
...
function shouldWindStance(availableStances = unlockedStances(), baseStats = getBaseStats(), currentEnemy = getCurrentEnemy()) {
	if (game.global.mapsActive || !availableStances.includes('W') || !['Wind', 'ALL'].includes(getUberEmpowerment()) || getEmpowerment() !== 'Wind') return false;
...
function scryTransition(scryStance = 'S', scrySettings = scrySettings(), baseStats = getBaseStats(), availableStances = unlockedStances(), transitionRequired = false, currentEnemy = getCurrentEnemy()) {
